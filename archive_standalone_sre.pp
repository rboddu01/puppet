...
class jboss::archive_standalone_sre (
  ...
  # Removes standalone-full-ha file, if any manual modifications on file
  exec { 'standalone file compare':
    command  => "rm -rf ${standalone_full_ha_file} $dep_valid/datasource*",
...
$password = undef,
  ...
  $cmd = "sh ${jboss::install_path}/${jboss::app_module}/configuration/dep_valid/standalone-full-ha.xml_updates.sh \'${toFile}\' \'${jndi_name}\' \'${pool_name}\' \'${connection_url}\' \'${driver}\' \'${min_pool_size}\' \'${max_pool_size}\' \'${security_domain}\' \'${username}\' \'${password}\'"

  $dcname = $name.split(':')[-1]
  $dep_validpth = "${jboss::install_path}/${jboss::app_module}/configuration/dep_valid"
  $standalone_full_ha_file = "${jboss::install_path}/${jboss::app_module}/configuration/standalone-full-ha.xml"

  file { "$dep_validpth/$dcname.status":
    ensure    => present,
    content   => "$datasource00_enabled - $cmd",
    require   => File["$dep_validpth/standalone-full-ha_prime_source.xml"],
    notify    => Exec["Create $standalone_full_ha_file"]
  }

  if $datasource00_enabled {
    if $jndi_name and $pool_name and $connection_url and $driver and $security_domain {

      exec { $cmd:
        refreshonly => true,
        #unless  => "grep -i \"datasource.*.jndi-name.*.${jndi_name}.*\" $toFile",
        require     => File["$dep_validpth/standalone-full-ha.xml_updates.sh"],
        subscribe   => [ File["$dep_validpth/$dcname.status"], File["$dep_validpth/standalone-full-ha_prime_source.xml"] ]
      }
    } else {
...
$password = undef,
  ...
  file { "$dep_validpth/$dcname.status":
    ...
    content => "$datasource01_enabled - $cmd",
    require   => File["$dep_validpth/standalone-full-ha_prime_source.xml"],
    notify    => Exec["Create $standalone_full_ha_file"]
  }

  if $datasource01_enabled {
    if $jndi_name and $pool_name and $connection_url and $driver and $security_domain {
...
Boolean $datasource02_enabled  =  false,
$jndi_name = undef,
$pool_name = undef,
$connection_url = undef,
$driver = 'dhw.com.oracle',
...
$password = undef,
  ...
  $dep_validpth = "${jboss::install_path}/${jboss::app_module}/configuration/dep_valid"
  $standalone_full_ha_file = "${jboss::install_path}/${jboss::app_module}/configuration/standalone-full-ha.xml"
  $standalone_full_ha_file_back = "$dep_validpth/standalone-full-ha.xml_back"

  notify {"hello $standalone_full_ha_file  -> $standalone_full_ha_file_back":}
  file { "$dep_validpth/$dcname.status":
    ensure  => present,
    content => "$datasource02_enabled - $cmd",
    require => File["$dep_validpth/standalone-full-ha_prime_source.xml"],
    notify  => [ Exec["Take backup of $standalone_full_ha_file"] ]
  }

  if $datasource02_enabled {
    if $jndi_name and $pool_name and $connection_url and $driver and $security_domain {
